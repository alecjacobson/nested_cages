...
public:
    ...
    static bool detectCollisions(const std::vector<RigidBodyInstance *> &bodies, std::vector<Plane> &planes, std::vector<Eigen::Vector3d> &newc, std::vector<Eigen::Vector3d> &newtheta, std::vector<ContactInfo> &contacts, SimParameters::UseCage  useCage);
