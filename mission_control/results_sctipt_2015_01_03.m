...
% [cages_V,cages_F,Pall,V_coarse,F_coarse,timing] = multires_per_layer(V0,F0,[floor(2^(-14/3)*size(F0,1)) floor(2^(-12/3)*size(F0,1)) floor(2^(-10/3)*size(F0,1)) floor(2^(-8/3)*size(F0,1)) floor(2^(-6/3)*size(F0,1)) floor(2^(-4/3)*size(F0,1)) floor(2^(-2/3)*size(F0,1))],'eps_distance',1e-3);
