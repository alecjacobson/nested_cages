function [V_coarse_final,timing,Pall_expansion] = ...
...
%     G = V0-V; %buggy
  ...
  params_to_variables = containers.Map( ...
    {  'SkipElTopo','ExpansionEnergy','FinalEnergy','EpsExpansion','EpsFinal',...,
    'BetaInit','Debug','Fquad','D_CV_MIN','RefCage','BETA_MIN','ShrinkPoints'}, ...
    {'skip_el_topo','energy_expansion','energy_final','eps_distance_expansion','eps_distance_final',...
    'beta_init','debug','Fquad','D_CV_MIN','CV_ref','BETA_MIN','shrink_points'});
  v = 1;
  ...
  % number of steps
  k = size(P_all,3);
  % initialize Pall_expansion
  Pall_expansion = zeros(size(CV,1),size(CV,2),k-1);
  ...
  for t = k-1:-1:1
    ...
    V = P_all(:,:,t);
        [CV_filtered,timing.per_step{k-t}] = one_step_project( ...
          ...
          CV_filtered,CF, ...
          energy_gradient,energy_value,cb_data, ...
          'BetaInit',beta,'Eps',eps_distance_expansion,...
          'SkipElTopo',skip_el_topo, ...
          'Tol',tol_dt,'PlotInfo',plot_info,'Debug',debug,...
          'D_CV_MIN',D_CV_MIN,'BETA_MIN',BETA_MIN,'ShrinkPoints',shrink_points);
    Pall_expansion(:,:,t) = CV_filtered;
  end
  ...
  if ~strcmp(energy_final,'none')
    ...
    [CV_filtered,timing.per_step{k}] = ...
      one_step_project( ...
        ...
        energy_gradient,energy_value,cb_data, ...
        'SkipElTopo',skip_el_topo, ...
        'BetaInit',beta,'Eps',eps_distance_final, ...
        'Tol',tol_dt,'PlotInfo',plot_info,'Debug',debug,...
        'D_CV_MIN',D_CV_MIN,'BETA_MIN',BETA_MIN,'ShrinkPoints',shrink_points);
    Pall_expansion(:,:,2:end+1) = Pall_expansion;
    Pall_expansion(:,:,1) = CV_filtered;
  end
