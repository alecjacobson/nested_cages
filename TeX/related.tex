...
boundaries.
%
\alec{previous multi-grid methods handle complicated boundaries
...
%
\alec{Some methods that \emph{do} use replicating in 3D: 
...
\alec{collision detection schemes do not need intermediary triangle so isosurface
of distance fields are OK for that application.}
...
%
\cite{Ben-Chen:2009:SDT} create exterior cages
...
\alec{Not fair, since ours will not converge.}
%
Further, a single-layer cage created with our
method is much tighter fitting for the same vertex count (see
\reffig{dane-vs-ben-chen}).
%
Xian et al.\ 
construct automatic cages by simplifying dense voxelizations
\cite{Xian:2009},
stitching oriented bounding boxes
around vertex clusters \cite{Xian:2012tv}, or by offsetting coarse
on-surface triangulations of feature curves \cite{Xian:2013}.
%
\cite{Sander:2000:SC} attempt to keep coarse meshes exterior during decimation
by enforcing local inequality constraints.
%
\cite{Deng:2011vr}
use a similar construction and resort to post-hoc global
intersection resolution to repair invalid output cages.
%
...
%
\alec{Incorporating decimation into our pipeline is tempting. The ability to
add or remove extra degrees of freedom according to the tightness of fit or
difficulty to maintain nestingness is attractive. To this end, we experimented
with subdivision, but then it was difficult to ensure that our output stays
coarse. Nevertheless, this could be interesting future work and may be
necessary in some impossible configurations (see Csasar counterexample)}
...
%
\alec{ \cite{Volino:2006:RSC} as alternative to re-inflation using
Eltopo/SPACM, here, or in method. Also could lead to wrong ordering?}
\alec{From rebuttal: ``quickly gets stuck in local minima if the decimation
mesh cuts through geometrically complex features of the fine mesh. Also will
fail to converge in concave regions if pushing the coarse mesh out of the fine
causes coarse mesh self-collisions, and vice versa."}
%
...
\alec{Cite enveloping in ``Abstraction of Man-Made Shapes''}

\alec{reference surface-based multi-resolution methods
   \cite{Aksoylu2005msu,Chuang:2009:ELO}}

\alec{reference recent collision detection papers \cite{BEB2012,Wang:2014:DCC}: I
