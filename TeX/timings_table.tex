% !TEX root = winding.tex
\begin{table}
\centering
\ra{1.2}
\setlength{\tabcolsep}{5.5pt}
\rowcolors{2}{white}{lightbluishgrey}
\begin{tabular}{l r r r r l r r r r r r r r r r r r}
\rowcolor{white}
Model name  & \#F & k & avg($t_{flow}$) & avg($t_{re}$) & Energy\\
\midrule
S.W.A.T. & 9,820 & 1 & 5s & 269s & Symmetry \\
Anchor & 10,778 & 6 & 6s & 43s & $E_\text{vol}$ \\
%Armadillo & 12,000 & 6 & 31s & 998s & $E_\text{varap}$ \\
Gargo & 13,500 & 7 & 2s & 90s & $E_\text{varap}$\\
% Model9
%Gallop & 16,843 & 1 & 3s & 268s & $E_\text{sarap}$  \\
Warrior & 26,658 & 7 & 2s & 86s & $E_\text{varap}$  \\
%Hand & 28,690 & 7 & 4s & 261s & $E_\text{varap}$  \\
%Animal & 39,036 & 7 & 9s & 1750s & $E_\text{sarap}$ \\
Pelvis & 40,316 & 7 & 11s & 460s & $E_\text{vol}$  \\
Bunny & 52,910 & 7 & 11s & 202s & $E_\text{varap}$ \\
Mug & 74,720 & 7 & 7s & 54s & $E_\text{vol}$ \\
Octopus & 500,000 & 11 & 13s & 63s & $E_\text{step}$ \\
\bottomrule
\end{tabular}
\caption{
We report the average time per cage to flow ``avg($t_{flow}$),'' and to
re-inflate ``avg($t_{re}$)''.
\alec{The octopus here is not the same as the octopus used in the multires
experiment.} \leo{The octopus timings here are for \emph{octopus-500-disppath$\_$final}
in the \emph{Meshes/Results} folder. Alec, can you please double-check you used this one
for the multires experiment?}
}
\label{tab:timings}
\end{table}
