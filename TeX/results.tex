...
memory.
\alec{``Timings should be better explained."}
%
...
%
\alec{Explain why back substitution is so slow. Are factors dense?}
%
...
solved.
%
\alec{This is even true for inhomogeneous systems where local metrics are
varying between solves. In this case, the internal tet-meshing might need to be
recomputed (XXXsec for \emph{Octopus}), but our cages can be reused.}
%
\alec{Reference and compare to algebraic multigrid \cite{ruge1987algebraic}}
%
\alec{what are the transfer operators used in multiresolution results? Appendix
for multiresolution implementation? We use linear interpolation as prolongation
operator and its transpose as the restriction operator \cite{Demmel04}.}
%
\alec{``Compare to direct solver on machine with more memory?'' No, there will
always be a problem size big enough that direct solver will choke: This is just
...
%
\alec{Give simple example of naive decimation failing to converge}
%
\alec{multiresolution results seem to suggest that without this method
...
constant interpolation of nearest vertex, linear interpolation of closest point
on nearest face. Bottom line, extrapolation is not $C^0$, and some amount of
cleverness is needed to avoid discontinuities.}
...
numerics.
%
\alec{Problem statement did not include conditions on output mesh quality or
coarseness.}
%
...
Another application area for nested cages is collision detection: if an object
...
%
\alec{Need to clarify that these are rigid objects.}
%
...
%
\alec{Discuss that nesting is not maintained (cages do not \emph{track}) during
non-rigid deformation of fine mesh (cf.\ simulation meshes where coarse mesh is
deformed).}
...
\alec{Martin et al.\ use general polyhedral domains. Not just theoretical...}
%
...
\alec{``Tone down homeomorphism requirement: topological noise.'' Mention here
that SWAT is not strictly homeomorphic to cage as it contains topological
noise, but it is homeomorphic to the solid the SWAT mesh abstractly represents:
a topological sphere. This leads into the point cloud examples.}

\alec{``How can point clouds/polygon soups be handled?''}

\alec{``Would have appreciated seeing (more) coarser cages."}
